  ...
  if (!snapshot || now - lastVersionCheck >= RANKINGS_CACHE_REFRESH_MS) {
    ...
    if (!refreshing) {
      refreshing = (async () => {
        ...
        if (!fetchedAt) throw new Error("Ranking metadata is missing fetched_at.");
        if (!snapshot || fetchedAt !== snapshot.fetchedAt) {
...
export async function assertRankingsReady() {
  if (!readiness) readiness = (async () => {
    ...
    const [tableRows, columnRows, indexRows] = await Promise.all([
      query<{ name: string }>("SELECT table_name AS name FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN (?, ?)", tables),
