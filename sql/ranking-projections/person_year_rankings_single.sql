CREATE TABLE person_year_rankings_single AS
WITH country_candidates AS (
  SELECT
    ...
    result_id, competition_start_date, competition_id, best AS result_value,
    ROW_NUMBER() OVER (
  ...
  FROM (
    SELECT *, ROW_NUMBER() OVER (
      PARTITION BY ranking_year, event_id, person_id, continent_id
  ...
  FROM (
    SELECT *, ROW_NUMBER() OVER (
      PARTITION BY ranking_year, event_id, person_id
      ORDER BY result_value, competition_start_date, competition_id, result_id
    ) AS cohort_position
    FROM country_bests
...
SELECT
  candidate.ranking_year AS year, candidate.event_id, cohort.cohort_id,
  candidate.person_id, candidate.result_id, candidate.result_value,
  RANK() OVER (
    PARTITION BY candidate.ranking_year, candidate.event_id, cohort.cohort_id
...
JOIN person_year_ranking_cohorts cohort
  ON cohort.scope = candidate.scope AND cohort.region_id = candidate.region_id;
...
ALTER TABLE person_year_rankings_single
  ...
  ADD INDEX idx_person_year_single_person (year, event_id, cohort_id, person_id);
