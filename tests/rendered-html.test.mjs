...
test("uses the copied WCA Rankings visual language", async () => {
  ...
  assert.match(css, /\.resultValue \{[\s\S]*display: grid;[\s\S]*grid-template-columns: 8ch 5\.5ch;/);
  assert.match(css, /\.competitionName \{[\s\S]*width: 8ch;[\s\S]*margin-right: calc\(5\.5ch \+ 0\.5em\);/);
